...
ll root, node_num, n, m, last = 0, ans = 0;//根节点编号  节点个数
...
int main() {
    ...
    for (ll i = 1; i <= m; ++i) {
        ll operate = readf<ll>(), x = readf<ll>() xor last;
        switch (operate) {
            //向 M 中插入一个数 x
        case 1:
            insert(x);
        ...
        case 3:
            //printf("%lld\n", last = get_Rank_By_Value(x));
            ans ^= last = get_Rank_By_Value(x);
#if DEBUG
            printf("%lld now is  root value is: %lld\n", i, tree[root].value);
            show(root);
#endif
        ...
        case 4:
            //printf("%lld\n", last = get_Value_By_Rank(x));
            ans ^= last = get_Value_By_Rank(x);
#if DEBUG
            printf("%lld now is  root value is: %lld\n", i, tree[root].value);
            show(root);
#endif
