...
namespace Splay {
    ...
    inline void update_size(ll _Index); //在改变节点位置后，将节点 _P 的 size 更新。

    inline bool get_child(ll _Index); //判断节点 x 是父亲节点的左儿子还是右儿子。 左false 右true

    inline void remove(ll _Index);

    inline void rotate(ll _Index); //将点_Index和其父节点旋转
}
